...
{
    class DaimlerTestNoAAWidth1 : TestScene
    {
        public DaimlerTestNoAAWidth1() : base(
            TestCategory.CT_CustomerTests,
            "Daimler: NO AA Width 1 Line Plot defined with large set of points",
            @"Issue from Daimler.
Black Plot is defined with 2000 points. No antialiased, 1px Width, Dashed Pattern.

Red Plot is a DOWNSAMPLED version of Black Plot defined with 80 points. No antialiased, 1px Width, Dashed Pattern.
...
Red and Black Plot should be drawn without any artefacts using both renders GDI and OpenGL")
        ...
        public override Scene GetScene()
        {
            ...
            {
                new LinePlot(X: X, Y: V, lineStyle: DashStyle.PointDash)
                {
                    Line = { Antialiasing = false, Color = Color.Black, Width = 1 }
                },

                new LinePlot(X: linspace(-pi, pi, downSampleFactor), Y:downSampledCurve+1, lineStyle: DashStyle.PointDash)
                {
                    Line = { Antialiasing = false, Color = Color.Red, Width = 1 }
                },
