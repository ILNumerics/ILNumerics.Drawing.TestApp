...
{
    class LinePlotAAWidthDashedPlotCube : TestScene
    {
        public LinePlotAAWidthDashedPlotCube() : base(
            TestCategory.RA_AntialiasedLines,
            "Anti-Aliased Dashed Lines Drawing in PlotCube",
            @"[TestCase]:
...
Check if lines edges are correctly anti-aliased(blured).")
        ...
        public override Scene GetScene()
        {
            ...
            {
                new LinePlot(tag: "myPlot", positions: linearray, lineStyle: DashStyle.Dashed)
                {
