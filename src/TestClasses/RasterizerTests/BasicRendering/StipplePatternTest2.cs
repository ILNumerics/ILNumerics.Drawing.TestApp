...
{
    class StipplePaternTest2 : TestScene
    {
        public StipplePaternTest2() : base(
            TestCategory.RA_BasicRendering,
            "Stipple Patern Rendering Width 1",
            @"Two dash dotted lines are drawn(width = 1px). 
...
Try to resize the window, stipple pattern must be continued through the points.")
        ...
        public override Scene GetScene()
        {
            ...
            {
                new LineStrip("linestrip") {Positions = positions1.T ,Color = Color.Red,DashStyle = DashStyle.PointDash, Width = 1},
                new LinePlot(positions:positions2.T, lineStyle:DashStyle.PointDash, lineWidth:1)
                {
